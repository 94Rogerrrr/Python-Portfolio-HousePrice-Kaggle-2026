...
- 多種樹模型之預測誤差小但是 rmse gap 均明顯大於線性模型，故推論房價資料教適合以線性模型預測，可以兼顧最小的均方誤差與過度擬合問題。
...
- Canditate Model (Random Forest Regression):
    ...
    - rmse : 0.14
    - rmse_gap : 1.90
    - 已先透過 GridSearchCV 找出該批資料中最適切的參數，仍得出過度擬合的結果，並且測試集的 rmse 略高於 Linear Regression。顯示本次資料轉碼後的結果，對於隨機森林模型有較大的限制。如為了降低過度擬合，會導致預測模型失準。
- Canditate Model (XGBoost Regression):
    - r^2 : 0.90
    - rmse : 0.12
    - rmse_gap : 3.45
    - 已先透過早停法、CV k fold 方式，找出該批資料中最適切的參數，名稱為 CVxgbModel。rmse gap 雖然比沒使用 CV k fold 時低許多，但預測結果仍是過度擬合。測試集的 rmse 等於 Linear Regression。顯示本次資料較不適合使用 XGBoost 。
- Canditate Model (LightGBM Regression):
    - r^2 : 0.91
    - rmse : 0.12
    - rmse_gap : 0.95
    - 已先透過 GridSearchCV 找出該批資料中最適切的參數，仍得出過度擬合的結果，測試集的 rmse 等於 Linear Regression。顯示本次資料較不適合使用 LightGBM 。
- Final Model (LassoCV): RMSE 0.12 (**提升 33%**)
    - r^2 : 0.91
    - rmse : 0.12
    - rmse_gap : 0.14
    - 此模型測試集等於 Linear Regression 不過 rmse gap 略低於 Linear Regression 故選定期為最終勝出模型。
